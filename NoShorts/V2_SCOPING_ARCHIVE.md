...
- **NextDNS on the owner's phone may itself be a DNS profile.** If so, iOS DNS profile priority rules apply — the most-recently-installed profile might win, or system might allow multiple domain-scoped profiles to compose. We won't know without trying.
...
5. Install profile on the owner's phone.
6. Install V2 on the owner's phone (fresh bundle ID `com.deviationlabs.NoShortsV2` to coexist with V1 for A/B comparison, then swap once confirmed).
7. Verify.
...
- **Users**: right now, one — the owner. Not a public product. Not App Store. Dev-signed / TestFlight scope only.
...
- **Legal / TOS**: this pattern (extractor-based YouTube client) violates YouTube TOS. All open-source alternatives operate in this same gray zone. Not distributed to App Store. the owner accepts this.
...
- Con: YouTube changes their extractor-facing surface every few weeks. Someone has to patch. This is the well-known "extractor treadmill."
...
**Decision needed from the owner:** A, B, or C? My default recommendation is **B (Yattee-style, Piped backend)**. Reasons:

- the owner already has a track record of maintaining IoT projects (homely_vibes). Adding "keep YouTube extractor working" to that list is real burden.
- The whole session today shows the owner values *working* over *perfectly-architected-but-fragile*.
...
If the owner prefers self-hosting the extraction: pick B and plan to run a personal Piped instance.
...
the owner confirm the MVP list matches your intent.
...
16. Is V1 archived / kept installed as a fallback, or actively deleted from the owner's phone?
...
- **M3 — Ship internally:** installed on the owner's phone via Xcode, used daily for a week.
...
## 9. Questions to the owner — resolve these before I write code
