...
NoShorts is a personal iOS app (single user, single device).

It exists to solve one narrow problem: **The owner deliberately blocks `*.youtube.com` at the DNS layer on the phone via NextDNS** so that Safari, the YouTube app, and every other channel to YouTube on the device fails DNS resolution and shows an error. This is intentional friction to reduce their own YouTube usage — especially Shorts, algorithmic feed, and doom-scrolling.

NoShorts is the sanctioned escape hatch: **the only path on the phone that reaches YouTube**, and only in a shape the owner endorses — Playlists as the landing page instead of the algorithmic home, Shorts hidden across the app, a 30-minute session timer, forced landscape on `/watch`, autoplay off.
...
- Single owner-operator. Modern iPhone on iOS 26.5.2 (the version the playback regression was diagnosed on), residential Wi-Fi at home and cellular elsewhere. Personal Apple Developer signing (free tier).
...
- N6. Any dependency on a hosted service the owner doesn't run (no Piped, no yt-dlp on Vercel, no Cobalt). Zero external ops surface for MVP.

### Success criteria (definition of done)

- On the owner's iPhone with NextDNS active, opening NoShorts and tapping a video plays it.
...
- We must not require the owner to install a device-wide DNS override profile, because that would defeat G2.
...
- **Zero external services.** No Piped, no yt-dlp on a VPS, no the home server dependency. If it works, MVP has one moving part (the app) and one static config (dns.google DoH endpoint).
...
the owner approves either (a) clean up V1 in place or (b) delete `NoShorts/NoShorts/*.swift` and rewrite from scratch. Decision belongs to the owner. Everything below is written assuming (b) — from scratch — because the V1 code has accumulated JS wrappers, dead reverts, and workarounds that are more expensive to comprehend than replace.
...
Install a device-wide `.mobileconfig` DoH override for `*.youtube.com`. This defeats G2 (Safari can then reach YouTube too), but it's honest and it works. NoShorts becomes a thin UX layer instead of a discipline device. the owner's call whether to accept this trade-off.
...
**Explicit non-plan:** we do NOT chase the "self-hosted yt-dlp on the home server" path unless Tier 4 is also unacceptable AND the owner signs up for the ongoing maintenance treadmill. It's over-scoped for MVP.
...
- Diagnosis: proxy path is fine. **WebContent's direct fetches to `googlevideo.com` (ISP-hosted Google Global Cache edges, e.g. a large residential ISP block) cannot complete** from this third-party app on iOS 26.5.2. This is the §7 Tier 2 signal — the failure is in WebContent, not in the LocalProxy.
...
- Self-hosted `yt-dlp` on the home server (a residential-IP home server), exposed via Cloudflare Tunnel or similar. Real ops surface but removes the "third-party app can't reach googlevideo" problem entirely by making our app talk to the home server instead.
- Self-hosted Piped instance on the home server — same idea, heavier stack.
...
3. the owner decides self-hosting an extractor on the home server is worth the ongoing ops.
...
**V1's original break mode** — On iOS 26.5.2, LocalProxy CONNECTs to `youtube.com` (Google IPs) succeed, but CONNECTs to `googlevideo.com` (ISP/cellular-carrier GGC IPs) fail with `Network is down` or `Connection reset by peer` mid-tunnel. iOS 26.5.2 apparently blocks proxy-originated outbound TCP to non-Google IPs from a third-party app. Same behavior on residential Wi-Fi and cellular — different edge IPs, same failure.
...
- Self-hosted Piped on the home server — a real fallback but adds ongoing ops.
- Self-hosted `yt-dlp` HTTP wrapper on the home server (residential IP, so YouTube doesn't rate-limit) — same shape as Piped, less code but more upkeep.
