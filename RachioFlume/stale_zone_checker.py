...
Fires a P2 alert if any enabled controller zone or known hose-timer valve
...
P2 (emergency, retries until acked) rather than the usual health-check P1:
the window is long enough -- 15 days -- that anything reaching it is a real
watering failure, not a blip, and by then a zone has already missed a
fortnight of irrigation. The tuning history is the justification: 10 days
still fired often enough to be noise, and an alert that is routinely ignored
is not an alert. Widening the window is what earns the louder priority.
...
class StaleZoneChecker:
    ...
    def _maybe_notify(
        ...
        *,
        ...
        self.pushover.send_message("\n".join(lines), title="RachioFlume: Stale Zone", priority=2)
