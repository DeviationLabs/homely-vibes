...
- **Stale-zone monitor** — separate `StaleZoneChecker` fires **P2** if any
  ...
  valve battery, etc.
  The 10 → 15 day / P1 → P2 retune is one change, not two: at 10 days the
  alert fired often enough on ordinary seasonal gaps to be ignored, and an
  alert that is routinely ignored is not an alert. Widening the window is
  what makes the louder priority correct rather than merely louder.
