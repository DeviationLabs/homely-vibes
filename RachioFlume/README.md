...
- **Usage Alerts** (`alert_engine.py`):
  ...
  - **Stale-zone monitor** (`stale_zone_checker.py`): P1 alert if any enabled controller zone or **known hose-timer valve — connected or not** — hasn't run within `stale_zone_days` (default 10). A disconnected valve is exactly the failure mode that stops it running, so it is never filtered out; the alert body notes `DISCONNECTED` (battery/BLE) and roster-staleness (valve dropped from the API response) as extra actionable lines. Daily dedup; hourly evaluation gate.
